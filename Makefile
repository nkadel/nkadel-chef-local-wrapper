...
# Use --fqdn
HOST := `hostname --fqdn`
DEFAULTNODE := nodes-local/default.json.tmpl
HOSTNODE := nodes-local/$(HOST).json
# chef-local configuration to find working chef repo
LOCAL := local.rb

# 
LOCALNODE := nodes/node.json
...
node: $(HOSTNODE)
$(HOSTNODE):
	@ echo setting up $(HOSTNODE)
	@case $(HOST) in \
	    template1-*) \
		echo "Linking template1.json.tmpl template to $(HOSTNODE)"; \
		ln -sf ../nodes-local/template1.json.tmpl $(HOSTNODE); \
		;; \
	esac
...
$(LOCALNODE): $(HOSTNODE)
	@if [ -s "$(HOSTNODE)" ]; then \
	    echo "Installing $? to $@"; \
	    install -D -m 644 $(HOSTNODE) $@; \
	else \
	    echo "Warning: $(HOSTNODE) not found"; \
	    echo "    Installing $(DEFAULTNODE) as $@"; \
	    install -D -m 644 $(DEFAULTNODE) $@; \
	fi

.PHONY: $(COOKBOOKSLINK)
$(COOKBOOKSLINK): $(BERKSCOOKBOOKS)
	@echo Checking link: $@
	@if [ "`readlink $(COOKBOOKSLINK)`" != $(BERKSCOOKBOOKS) ]; then \
	    ln -sf $(BERKSCOOKBOOKS) $(COOKBOOKSLINK); \
	fi
...
run:: $(LOCALNODE)
	...
	@echo
	@echo "Running: $(CHEFLOCAL) --config $(PWD)/$(LOCAL) -j $(LOCALNODE), logs in chef-local.out"
	$(CHEFLOCAL) --config $(PWD)/$(LOCAL) -j $(LOCALNODE) 2>&1 | tee chef-local.fail.out

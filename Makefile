...
run:: $(NODECOPY)
	...
	@echo
	@echo "Running: $(CHEFLOCAL) --config $(PWD)/$(LOCAL) -j $(NODECOPY), logs in chef-local.out"
	$(CHEFLOCAL) --config $(PWD)/$(LOCAL) -j $(NODECOPY) 2>&1 | tee chef-local.fail.out

...
links: $(NODELINK) $(LOCALLINK) $(COOKBOOKSLINK)
...
$(LOCALLINK): $(LOCAL)
	@echo Checking link: $@
	@[ "`readlink --canonicalize $(LOCALLINK)`" = $(PWD)/$(LOCAL) ] || \
	       ln -sf $(PWD)/$(LOCAL) $(LOCALLINK)
...
run: links
	...
	@echo
	@echo "Running: $(CHEFLOCAL) --config $(LOCALLINK) -j $(NODELINK), logs in chef-local.out"
	$(CHEFLOCAL) --config $(LOCALLINK) -j $(NODELINK) 2>&1 | tee chef-local.fail.out
